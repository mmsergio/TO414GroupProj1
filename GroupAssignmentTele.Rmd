...
tele_norm_cl1_test$yyes <- predict(reg1, newdata = tele_norm_cl1_test, type = "response")
tele_norm_cl1_test$yyes <- ifelse(tele_norm_cl1_test$yyes > 0.5, 1, 0)
...
CrossTable(x = tele_norm_cl1_testlabels, y = tele_norm_cl1_test$yye, prop.chisq=FALSE)
...
tele_norm_cl2_test$yyes <- predict(reg2, newdata = tele_norm_cl2_test, type = "response")
tele_norm_cl2_test$yyes <- ifelse(tele_norm_cl2_test$yyes > 0.5, 1, 0)

tele_norm_cl2_testlabels <- tele_norm_cl2[test_cl2, "yyes"]

CrossTable(x = tele_norm_cl2_testlabels, y = tele_norm_cl2_test$yye, prop.chisq=FALSE)
...
tele_norm_cl5_test$yyes <- predict(reg5, newdata = tele_norm_cl5_test, type = "response")
tele_norm_cl5_test$yyes <- ifelse(tele_norm_cl5_test$yyes > 0.5, 1, 0)

tele_norm_cl5_testlabels <- tele_norm_cl5[test_cl5, "yyes"]

CrossTable(x = tele_norm_cl5_testlabels, y = tele_norm_cl5_test$yye, prop.chisq=FALSE)
...
```{r}
neural_test$prediction <- predict(neuralmod, neural_test)
neural_test$prediction <- ifelse(neural_test$prediction > 0.5, 1, 0)
  #Add column with prediction
```
CrossTable(x = neural_test$prediction, y = neural_test$yyes, prop.chisq=FALSE)

confusionMatrix(as.factor(neural_test$prediction), as.factor(neural_test$yyes), positive = "1")
  #Compare yyes and prediction variable in the matrix
